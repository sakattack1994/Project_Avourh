...
function process_file($f){
  ...
  elseif(substr($name, -4)!=".txt"){
    ...
    unlink($_SERVER['DOCUMENT_ROOT']."/myDepartment/myresources/history.txt");
    move_uploaded_file($_FILES["file"]["tmp_name"],$_SERVER['DOCUMENT_ROOT'] .'/myDepartment/myresources/'.$_FILES['file']['name']);
    rename("myresources/".$_FILES['file']['name'],"myresources/history.txt");
    $f=1;
  }
  return $f;
}
  ...
  $page=fread($myfile,filesize("myresources/history.txt"));
  $content="<div class=\"col-md-9\"><div id=\"content\">
      ...
      <form enctype=\"multipart/form-data\" action=\"history.php\" method=\"post\">
        <label><h3>Replace the history page with a new txt file:</h3></label>
