...
function process_file($f){
  ...
  elseif(substr($name, -4)!=".txt"){
    ...
    unlink($_SERVER['DOCUMENT_ROOT']."/myDepartment/myresources/currentAcademicYear.txt");
    move_uploaded_file($_FILES["file"]["tmp_name"],$_SERVER['DOCUMENT_ROOT'] .'/myDepartment/myresources/'.$_FILES['file']['name']);
    rename("myresources/".$_FILES['file']['name'],"myresources/currentAcademicYear.txt");
    $f=1;
  }
  return $f;
}
  ...
  $content="<div class=\"col-md-9\"><div id=\"content\">
      ...
      <br>  <br>  <br>
      ".$alert."
      <form action=\"current_academic_year.php\" id=\"edit_page\" method=\"POST\">
      ...
      <form enctype=\"multipart/form-data\" action=\"current_academic_year.php\" method=\"post\">
        <label><h3>Replace the current_academic_year page with a new txt file:</h3></label>
