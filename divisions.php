...
function process_file($f){
  ...
  elseif(substr($name, -4)!=".txt"){
    ...
    unlink($_SERVER['DOCUMENT_ROOT']."/myDepartment/myresources/divisions.txt");
    move_uploaded_file($_FILES["file"]["tmp_name"],$_SERVER['DOCUMENT_ROOT'] .'/myDepartment/myresources/'.$_FILES['file']['name']);
    rename("myresources/".$_FILES['file']['name'],"myresources/divisions.txt");
    $f=1;
  }
  return $f;
}
  ...
  if(isset($_POST['edit'])){
    $txt=$_POST['edit'];
    $myfile = fopen("myresources/divisions.txt", "w") or die("Unable to open file!");
    ftruncate($myfile, 0);
    fwrite($myfile, $txt);
    fclose($myfile);
    $alert="<div class=\"alert alert-success\"><strong>The edit was successful.</strong></div>";
  }
  $myfile = fopen("myresources/divisions.txt", "r") or die("Unable to open file!");
  $page=fread($myfile,filesize("myresources/divisions.txt"));
  $content="<div class=\"col-md-9\"><div id=\"content\">
      ".$page."
      <br>  <br>  <br>
      ".$alert."
      <form action=\"divisions.php\" id=\"edit_page\" method=\"POST\">
      ...
      <form enctype=\"multipart/form-data\" action=\"divisions.php\" method=\"post\">
        <label><h3>Replace the divisions page with a new txt file:</h3></label>
